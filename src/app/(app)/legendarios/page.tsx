...
import { getFeaturedEvent } from "@/features/legendarios/data/featured-event";
          ...
          >
            ...
            {event.targetDate && (
              <div>
                ...
                <CountdownTimer target={event.targetDate} accentColor={ORANGE} />
              </div>
            )}
            ...
            {event.steps.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-left">
                {event.steps.map((s, i) => (
                  <div key={i} className="p-4 rounded-xl" style={{ background: ORANGE_BG, border: `1px solid ${ORANGE_BORDER}` }}>
                    <span
                    ...
                    <p className="text-sm font-bold" style={{ color: ORANGE_L, fontFamily: "var(--font-cinzel)" }}>{s.title}</p>
            ...
            {event.officialSiteUrl && (
              ...
              >
                {event.officialSiteUrl.replace(/^https?:\/\//, "").replace(/\/$/, "")}
              </a>
            )}
          </div>
        </div>
